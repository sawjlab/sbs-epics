...
def xl2sub(csvfile):
    ...
    fh = open(rfilename,"w")
    print('#',file=fh)
    print('# This file was autogenerated by the command \"'+__file__+' -csvfile '+csvfile+'\" on: '+today, file=fh)
    print('# To change the HV channels, edit '+csvfile+' and rerun the commnd, remake the database and restart the ioc',file=fh)
    print('#',file=fh)
    ...
    print('file \"db/caenchassis.db\" {',file=fh)
    print("    pattern { Cr, CrName, CScode }", file=fh)
    print('\t{ "%d", "%s", "#C%d" }'%(CRATE, CRNAME, CRATE), file=fh)
    print('}', file=fh)
    print('file \"db/caenhv.db\" {',file=fh)

    print('\tpattern { Cr,   CrName,    CrType, Sl,   Ch,   Sys,  Det,    Element, CScode, ',end='',file=fh)
    print(', '.join([x for x in command_codes]),end='', file=fh)
    print(',',', '.join([x for x in alarm_limits])+"}", file=fh)
